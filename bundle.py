...
nyaanlocal_flag = False

for line in lines:
    """
    # NyaanLocal 部分を適切に削除 -> やっぱやめる
    if re.search("#ifdef NyaanLocal", line):
        nyaanlocal_flag = True
        continue
    if nyaanlocal_flag == True and re.search("#elif", line):
        print("don't use #ifdef NyaanLocal ~ #elif", file=sys.stderr)
        raise RuntimeError
    if nyaanlocal_flag == True and re.search("#else", line):
        print("don't use #ifdef NyaanLocal ~ #else", file=sys.stderr)
        raise RuntimeError
    if nyaanlocal_flag == True and re.search("#endif", line):
        nyaanlocal_flag = False
        continue
    """
    ...
    if re.search("define trc", line) and not re.search("define trc2", line):
        if re.search("void", line) == None:
    ...
    if nyaanlocal_flag or debug_flag or trc_ifdef_flag or trc2_ifdef_flag:
        continue
