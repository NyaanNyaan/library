...
data:
  ...
  bundledCode: "#line 1 \"verify/verify-yosupo-ds/yosupo-vertex-add-path-sum.test.cpp\"\
  ...
  code: "#define PROBLEM \"https://judge.yosupo.jp/problem/vertex_add_path_sum\"\n\
    ...
    \ void Nyaan::solve() {\n  ini(N, Q);\n  vl a(N);\n  in(a);\n  auto g = graph(N,\
    \ N - 1, false, false);\n\n  HeavyLightDecomposition<vvi> hld(g);\n  auto f =\
    \ [](ll a, ll b) { return a + b; };\n  SegmentTree<ll, decltype(f)> seg(N, f,\
    \ 0);\n  rep(i, N) { seg.set(hld.idx(i).first, a[i]); }\n  seg.build();\n\n  ll\
    \ ans = 0;\n  auto que = [&](int u, int v) { ans += seg.query(u, v); };\n\n  rep(_,\
    \ Q) {\n    ini(cmd, u, v);\n    if (cmd) {\n      ans = 0;\n      hld.path_query(u,\
    \ v,true, que);\n      out(ans);\n    } else {\n      seg.add(hld.idx(u).first,\
...
layout: document
redirect_from:
- /verify/verify/verify-yosupo-ds/yosupo-vertex-add-path-sum.test.cpp
- /verify/verify/verify-yosupo-ds/yosupo-vertex-add-path-sum.test.cpp.html
