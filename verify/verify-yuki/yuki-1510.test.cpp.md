...
data:
  ...
  attributes:
    ...
    PROBLEM: https://yukicoder.me/problems/no/1510
    links:
    - https://yukicoder.me/problems/no/1510
  bundledCode: "#line 1 \"verify/verify-yuki/yuki-1510.test.cpp\"\n#define PROBLEM\
    ...
    \ Nyaan {\nvoid solve();\n}\nint main() { Nyaan::solve(); }\n#line 4 \"verify/verify-yuki/yuki-1510.test.cpp\"\
    ...
    \ { return mod; }\n};\n#line 8 \"verify/verify-yuki/yuki-1510.test.cpp\"\n\nusing\
    \ namespace Nyaan;\n\nusing mint = LazyMontgomeryModInt<998244353>;\nusing vm\
    \ = vector<mint>;\nusing vvm = vector<vm>;\nusing fps = FormalPowerSeries<mint>;\n\
    \nvoid Nyaan::solve() {\n  int N;\n  in(N);\n  vi a(N);\n  for (auto& x : a) in(x);\n\
    \  map<int, int> mp;\n  each(x, a) mp[1LL * x * x % 998244353] = x;\n\n  map<int,\
    \ int> cnt;\n  each(x, a) cnt[x]++;\n\n  V<pair<mint, int>> denom;\n  each2(m,\
    \ d, cnt) { denom.emplace_back(mint(m) * m, d); }\n\n  auto part = PartialFractionDecomposition<mint>(fps{1},\
    \ denom);\n\n  vm co(210);\n  co[0] = 1;\n  for (int i = 1; i <= 202; i++) {\n\
    \    co[i] = co[i - 1] * (i * 2 - 1) / (i * 2);\n  }\n\n  mint ans = 0;\n  each2(m,\
    \ coeff, part) {\n    ll A = mp[m.get()];\n    for (int d = 1; d <= sz(coeff);\
  ...
  verifiedWith: []
documentation_of: verify/verify-yuki/yuki-1510.test.cpp
layout: document
redirect_from:
- /verify/verify/verify-yuki/yuki-1510.test.cpp
- /verify/verify/verify-yuki/yuki-1510.test.cpp.html
