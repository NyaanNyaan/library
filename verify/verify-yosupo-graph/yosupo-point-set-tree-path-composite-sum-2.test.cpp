...
//
#include "../../tree/static-top-tree-rerooting.hpp"
//
...
vector<mint> calc(int N, int Q, vector<int> A, vector<int> U, vector<int> V,
  ...
  RerootingDPonStaticTopTree<vector<vector<int>>, Path, Point, decltype(vertex),
...
void Nyaan::solve() {
  ...
  auto ac = calc(N, Q, A, U, V, B, C, cmd, I, X, Y, R);
