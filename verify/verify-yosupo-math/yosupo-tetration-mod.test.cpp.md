...
data:
  ...
  bundledCode: "#line 1 \"verify/verify-yosupo-math/yosupo-tetration-mod.test.cpp\"\
    ...
    \ 2 \"modulo/tetration.hpp\"\n\n\n\ntemplate <typename T = uint32_t, typename\
    \ U = uint64_t, T MAX = 1000000000>\nT tetration(uint64_t a, uint64_t b, uint64_t\
    \ m) {\n  auto prime_table = [](T m) -> vector<int> {\n    T thres = sqrt(m) +\
    \ 1;\n    vector<bool> flg(thres + 1);\n    for (T i = 2; i * i <= thres; ++i)\
    \ {\n      if (!flg[i])\n        for (T j = i * i; j <= thres; j += i) flg[j]\
    \ = 1;\n    }\n    vector<int> ret;\n    for (T i = 2; i <= thres; i++)\n    \
